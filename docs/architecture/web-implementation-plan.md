...
- 已存在 `app-web`
- 已存在契约生成模型 `app-web/src/contracts/generated/models.ts`
- 尚未建立完整 Next.js / React 工程
- 尚未实现 Web 网络层、登录态、页面路由和业务模块
...
因此 Web 端开发的重点不是重新探索业务，而是：

- 建立 Web 工程基线
- 复用跨平台契约
- 实现统一网络与 session 行为
- 完成 AI 主链路
- 建立 contract tests 和 Web 端回归保护
...
| 里程碑 | 目标 | 结果 |
| --- | --- | --- |
| M1 | 工程可运行 | Next.js 骨架、路由、样式、环境配置完成 |
| M2 | 登录闭环 | login / refresh / logout / session restore 完成 |
| M3 | AI 主流程 | AI 首页、对话、流式响应、历史详情完成 |
| M4 | 辅助模块 | history list、setting、privacy 完成 |
| M5 | 质量加固 | contract tests、E2E、构建、部署配置完成 |
| M6 | Web Beta | DEV / PRE 可用，进入真实联调和体验打磨 |

...
iOS、Android 和 HarmonyOS NEXT 已完成主要功能开发。HarmonyOS NEXT 当前默认连接 Apifox mock 环境；后续如需接入其它环境，应先更新跨端契约和网络配置。

## 当前执行状态

- [x] 阶段 1 已建立 `app-harmony` 源码骨架、模块边界文档、路由占位和页面占位。
- [x] DevEco 工程配置已迁入并可通过 hvigor 构建 Hap。
...
- [x] 阶段 9 已补 AIChat 链路。
- [x] 阶段 10 已完成 README、change log、AI 生成指南和工程卫生收尾。
...
- 先做可编译、可运行、可验证的链路，再追 UI 还原和体验细节。
...
- 明确当前默认连接 Apifox mock 环境；其它环境接入应通过契约和网络配置演进。
...
- 加载模板问题。
- 实现输入、发送、清空、新会话。
- 接 `/stream` mock 返回文本展示；HarmonyOS NetworkKit 实时 SSE / 打字机能力可作为后续体验优化。
- 图表 fallback 文案固定为：
...
- 当前已落地：
  ...
  - Apifox mock 模板问题、历史详情、函数分析、图表数据、`/stream` 文本和反馈状态。
...
- 运行 contract validation。
- 做工程卫生。
- 当前已完成；后续维护以 bugfix、UI 细节和 SSE 体验优化为主。

## 下一步建议

HarmonyOS NEXT 主要功能开发已完成。后续建议单独拆分为：

```text
bugfix / UI 细节 / SSE 体验优化 / Web 端生成
```

...
iOS 和 Android 已完成主要功能。HarmonyOS NEXT 第一版已完成学习项目主链路，当前连接 Apifox mock 环境，不连接真实生产后端。
...
- 明确第一版只连接 Apifox mock 环境，不连接真实生产后端。
...
- 加载模板问题。
- 实现输入、发送、清空、新会话。
- 接 `/stream` mock 返回文本展示；HarmonyOS NetworkKit 实时 SSE / 打字机能力不作为第一版验收项。
- 图表 fallback 文案固定为：
...
- 运行 contract validation。
- 做工程卫生。
- 当前已完成；真机能力、性能、签名和发布链路仍需设备侧单独验证。

## 下一步建议

HarmonyOS NEXT 第一版主链路已收尾。后续建议单独拆分为：

```text
真机验证 / 发布签名 / 实时 SSE 专项验证 / Web 端生成
```

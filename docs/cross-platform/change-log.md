...
## 2026-05-15 - HarmonyOS NEXT Stage 6 AIHome Shell

### Context

HarmonyOS NEXT 进入阶段 6：补 AIHome 壳层，承接登录成功后的 AI 业务入口、History 面板入口和 Setting 入口。

### Scope

- `app/pages/AIHomePage.ets` 从占位页升级为 authenticated module shell。
- `AIChatPage` 作为主 surface，接收 `selectedHistoryId` 和新会话版本。
- `HistoryPage` 作为辅助 overlay panel，提供关闭、设置入口和选择历史会话。
- `SettingPage` 增加返回入口，作为 App route 从 AIHome 打开。
- `pages/Index.ets` 增加 `AppDestination.Setting` 路由切换。

### Rule

- History 面板不能替换 AIChat 主 surface。
- 选择历史会话后关闭 History 面板，并在原 AIChat surface 加载对应 historyId。
- 新会话清空 `selectedHistoryId`，不 push 新 Chat route。
- 设置从 AIHome/History 入口进入 App route，返回后回到 AIHome。
...
## 2026-05-15 - HarmonyOS NEXT Stage 8 History Flow

### Context

HarmonyOS NEXT 进入阶段 8：补 History 列表链路，接 mock 数据、分组规则、无感刷新和选择会话。

### Scope

- 新增 `feature/history/application/HistoryRepository.ets`。
- 新增 `feature/history/application/HistoryRuntime.ets`。
- 新增 `feature/history/application/HistoryState.ets`。
- `HistoryPage` 从硬编码占位列表改为消费 `HistoryUiState`。
- 本地测试套件新增 History state tests。

### Rule

- History 面板打开时可以刷新数据，但已有数据时不能用全屏 loading 替换列表。
- 分组标题使用“今天 / 本月 / 其它”。
- 选择历史会话必须关闭 History 面板，并在现有 AIChat surface 加载对应 historyId。
- History 仍是 AIHome auxiliary panel，不是独立替换 AIChat 的主 route。
...
- Synced:
  ...
  - [ ] HarmonyOS NEXT
- Notes:
  - 默认开发环境统一为 Apifox mock host：`https://m1.apifoxmock.com/m1/3174267-1700689-default`。
  - 真实后端环境仍可通过平台环境配置覆盖。
...
- Impact:
  - HarmonyOS NEXT 阶段 1-10 已完成，主要功能开发完成。
  - `app-harmony` 当前连接 Apifox mock 环境，覆盖 Login、自动登录、AIHome、History、Setting、Privacy、AIChat、图表 fallback 和反馈状态。
  - 当前开源版本默认连接 Apifox mock 环境；后续如需接入其它环境，应先更新跨端契约和网络配置。
  - AIChat `/stream` 当前按完整响应解析 `data:` 内容并一次性展示；实时 SSE / 打字机效果可作为后续体验优化。
  - 清理了未使用的实时 stream transport 公共入口，避免把未保留的尝试性代码作为公共能力。
- Synced:
  - [x] HarmonyOS NEXT
  - [ ] Web
- Notes:
  - 本次是平台收尾，不改变跨端领域契约；后续维护以 bugfix、UI 细节、SSE 体验优化和 Web 端推进为主。
